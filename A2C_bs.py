...
Advantage Actor-Critic built by R.Ma
ver 1.0
"""
...
def actor_critic_baseline_subtraction_bootstrapping():
    hyp = Hyperparameters()
    device = ("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using {device} device is potentially faster")
    actor_model = Critic().to(device)
...
def plot_training_progress(scores, avg_scores, actor_losses, critic_losses):
    ...
    fig.suptitle('Training Progress of Actor-Critic with bootstrapping and baseline subtraction', fontsize=16)
...
if __name__ == '__main__':
    ...
    actor_critic_baseline_subtraction_bootstrapping()
